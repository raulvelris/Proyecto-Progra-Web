import React from "react";
import { ListGroup, Image } from "react-bootstrap";
import { VscGraph } from "react-icons/vsc";
import { FaUsers, FaHistory, FaCog, FaSignOutAlt } from "react-icons/fa";

const Barra: React.FC = () => {
  return (
    <div className="sidebar d-flex flex-column p-3">
      <Image
        src="https://a.espncdn.com/combiner/i?img=/i/headshots/soccer/players/full/45843.png&w=350&h=254"
        roundedCircle
        className="mb-3"
      />
      <h5 className="text-center">Lionel Messi</h5>
      <ListGroup variant="flush">
        <ListGroup.Item action className="text-secondary active">
          <VscGraph /> Dashboard
        </ListGroup.Item>
        <ListGroup.Item action className="text-muted">
          <FaUsers /> Usuarios
        </ListGroup.Item>
        <ListGroup.Item action className="text-secondary">
          <FaHistory /> Historial
        </ListGroup.Item>
        <ListGroup.Item action className="text-muted">
          <FaCog /> Configuración
        </ListGroup.Item>
        <ListGroup.Item action className="text-muted salir-item">
          <FaSignOutAlt /> Salir
        </ListGroup.Item>
      </ListGroup>
    </div>
  );
};

export default Barra;
